...
function ProtectedRoute({ permit,children }) {
    ...
    useEffect(()=>{
        if(auth && auth.profile && auth.profile.uid){
            setCheckedAuth(true);
        }
        if(auth && auth.error && auth.error.msg && !auth.profile){
            setCheckedAuth(true);
        }
    },[auth && auth.profile, auth && auth.error])

    return(
        checkedAuth?
            (auth && auth.profile && auth.profile.uid)?
